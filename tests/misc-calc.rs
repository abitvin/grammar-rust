...
fn misc_calc()
{
    ...
    grammar.add("num", "[0-9]+", Some(Box::new(|_, l| vec![l.parse().unwrap()])));
    grammar.add("brackets", "\\(<expr>\\)", None);
    grammar.add("mul", "(<num>|<brackets>)(\\*<mul>)?", Some(Box::new(|b, _| if b.len() == 1 { b } else { vec![b[0] * b[1]] } )));
    grammar.add("add", "<mul>(\\+<add>)?", Some(Box::new(|b, _| if b.len() == 1 { b } else { vec![b[0] + b[1]] } )));
